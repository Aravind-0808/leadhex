...
class WebklexImapEmailProcessor implements InboundEmailProcessor
{
    ...
    protected function processMessagesFromLeafFolders($rootFoldersCollection = null): void
    {
        $rootFoldersCollection->each(function ($folder) {
            // Skip 'All Mail' folder to avoid duplicates
            if (in_array($folder->name, ['All Mail'])) {
                return;
            }

            // Process messages in this folder (whether it has children or not)
            ...
            $messages->each(function ($message) {
                try {
                    ...
                    \Log::error('Failed to process email: ' . $message->getSubject() . ' - Error: ' . $e->getMessage());
                }
            });

            // Also process child folders recursively
